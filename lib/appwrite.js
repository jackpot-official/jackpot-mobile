...
export const removeFollowing = async (followerId, followedId) => {
    try {
        const following = await databases.listDocuments(
            appwriteConfig.databaseId,
            appwriteConfig.followingCollectionId,
            [Query.equal('user1', followerId), Query.equal('user2', followedId)]
        )

        if (following.documents.length > 0) {
            const followId = following.documents[0].$id
            await databases.deleteDocument(
                appwriteConfig.databaseId,
                appwriteConfig.followingCollectionId,
                followId
            ...
            return { message: 'Following relationship does not exist' }
        }
    } catch (error) {
...
export const getProfileUser = async (profileUserId) => {
    try {
        const response = await databases.listDocuments(
            appwriteConfig.databaseId,
            appwriteConfig.followingCollectionId,
            [Query.equal('accountId', [profileUserId])]
        )
